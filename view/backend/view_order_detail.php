<div class="col-md-10 col-xs-offset-1">
	...
	<?php 
		...
		$check = $this->model->fetch_one_array("select customer_id,trangthai from tbl_order where order_id=$order_id");
	...
	<?php 
		$customer = $this->model->fetch_one_array("select * from tbl_customer where customer_id=".$check["customer_id"]);
	 ?>
			...
			<table class="table table-bordered table-hover">
				...
				<?php 
					foreach($arr as $rows)
					{
						...
						$product = $this->model->fetch_one_array("select * from tbl_product where pk_product_id=".$rows["fk_product_id"]);
				 ?>				 
				<tr>
